...
class PaymentController {
    ...
    try {
      ...
      if (!payment) {
        return res
          ...
          .json({
            success: false,
            message: "No payment found matching criteria",
          });
      }
  ...
  async createPayment(req, res) {
    try {
      const { order_id, user_id, amount, payment_method, payment_status } =
        req.body;
      ...
      if (!order_id || !user_id || !amount || !payment_method) {
        return res.status(400).json({
          success: false,
          message:
            "Missing required fields: order_id, user_id, amount, and payment_method are required",
        });
      }
      ...
      res
        ...
        .json({
          ...
          order_id,
          amount,
          payment_method /*,transaction_id: payment.transaction_id*/,
